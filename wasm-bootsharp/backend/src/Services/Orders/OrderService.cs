...
public class OrderService(StoreDbContext dbContext, IShippingPriceCalculator shippingPriceCalculator) : IOrderService
{
    ...
    public async Task<int> CreateAsync(OrderDto.Create model)
    {
        ...
        var orderTotal = orderItems.Select(x => decimal.ToDouble(x.Product.Price.Value * x.Quantity)).Sum();
        var shippingCost = new Money((decimal)_shippingPriceCalculator.CalculateShippingPrice(orderTotal));
